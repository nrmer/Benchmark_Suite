...
#     timer = Synchronized_Benchmark_Timer(torch.npu.synchronize())
...
#     timer = Synchronized_Benchmark_Timer(torch.npu.synchronize(),timer_func=time.time())
#     timer = Synchronized_Benchmark_Timer(torch.npu.synchronize(),output_unit_conversion=10)
#     timer = Synchronized_Benchmark_Timer(torch.npu.synchronize(),output_unit='minutes')
#     timer = Synchronized_Benchmark_Timer(torch.npu.synchronize(),timer_func=time.time(), output_unit_conversion=10, output_unit='minutes')
...
def test_basic():
    ...
    dif = (ed - st) / 1e9
    print(abs(dif - timed))
    assert numpy.isclose(timed, dif, atol=1e-100, rtol=1e-4)